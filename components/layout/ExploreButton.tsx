...
export function ExploreButton() {
  ...
  return (
    <div className="explore-button-container mt-8 animate-fade-in-up">
          ...
          <div className="content">
            <span className="char state-1">
              {text1.split('').map((char, i) => (
                <span data-label={char} style={{'--i': i + 1} as React.CSSProperties} key={`s1-t1-${i}`}>{char}</span>
              ))}
              <span className="ml-2"></span>
              {text2.split('').map((char, i) => (
                <span data-label={char} style={{'--i': i + 1 + text1.length} as React.CSSProperties} key={`s1-t2-${i}`}>{char}</span>
              ))}
            </span>
            ...
            <div className="icon">
             ...
             <span className="char state-2">
              {text3.split('').map((char, i) => (
                <span data-label={char} style={{'--i': i + 1} as React.CSSProperties} key={`s3-${i}`}>{char}</span>
              ))}
            </span>
